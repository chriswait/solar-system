...
expect.extend({
  toBeNearMilliseconds(received, argument) {
    ...
    if (diff < 10) {
      return {
        message: () =>
          `expected ${received} not to be within 10 of ${argument}`,
        pass: true,
      };
    } else {
      return {
...
describe("should compute date of next tick", function () {
  ...
  it("with rate of 1 seconds", function () {
    let date = new Date();
    expect(Clock.getDateAfterNextTick(date, 1).getTime()).toBe(
      date.getTime() + 1 * 1000
    );
  });
  it("with rate of 1000 seconds", function () {
    let date = new Date();
    expect(Clock.getDateAfterNextTick(date, 1000).getTime()).toBe(
