...
const TargetCamera = () => {
  ...
  useFrame(() => {
    ...
    if (orbitControlsCameraRef?.current && targetCameraRef?.current) {
      targetCameraRef.current.copy(orbitControlsCameraRef.current);
    }
  });
  return <PerspectiveCamera makeDefault fov={VIEW_ANGLE} near={NEAR} far={ORBIT_MAX_UNITS * 4} ref={targetCameraRef} position={[0, 10, 10]} />;
};
