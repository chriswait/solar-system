...
const ClockPanel = () => {
  const { currentDate, julianDate, centuriesPastJ2000, clockRateSeconds, setClockRateSeconds } = useClock();
  return (
    <div id="clock-control-container" className="panel-container">
      <Panel title="Clock" reverse>
        <div className="row">
          ...
          <input id="clock-rate-seconds" value={clockRateSeconds} onChange={(event) => setClockRateSeconds(parseInt(event.target.value, 10))} />
        </div>
        <div className="row">
          <div>Date:</div>
